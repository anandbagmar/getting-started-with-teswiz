package com.znsio.sample.e2e.screen.web.playwrightjava.theapp;

import com.znsio.teswiz.runner.Driver;
import com.znsio.teswiz.runner.Visual;
import com.znsio.sample.e2e.screen.theapp.EchoScreen;

public class EchoScreenPlaywrightJava extends EchoScreen {
    private static final String ENGINE_NAME = "playwright-java";

    public EchoScreenPlaywrightJava(Driver driver, Visual visually) {
    }

    @Override
    public EchoScreen echoMessage(String message) {
        throw unsupported();
    }

    private UnsupportedOperationException unsupported() {
        return new UnsupportedOperationException(String.format(
                "TheApp Echo screen is not supported on web for WEB_ENGINE=%s.",
                ENGINE_NAME));
    }
}
